...
public class HiloParcela extends Thread {
    ...
    public void detenerPorLluvia() {
        if (regando) {
            ...
            this.interrupt();
            try {
                SistemaRespaldo1.liberarBomba(1);
            } catch (Exception e) {
    ...
    public void run() {
        while (activo) {
            ...
            try {
                ...
                double inr = SistemaRespaldo1.calculoINR(H, T, R);
                System.out.println("INR=" + inr + " parcela " + idParcela);

                if (SistemaRespaldo1.decidirRiego(llueve, inr)) {
                    try {
                        System.out.println("⏳ Parcela " + idParcela + " SOLICITA la bomba para riego...");
                        SistemaRespaldo1.solicitarBomba(SistemaRespaldo1.ESTADO_RIEGO, idParcela);

                        int minutos = SistemaRespaldo1.tiempoRiego(inr);
